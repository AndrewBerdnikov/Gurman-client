...
export const createRecipe = async (formData) => {
    const {data} = await $authHost.post('api/recipe/addRecipe', formData);
    return data;
}
...
export const getRecipes = async (page, limit) => {
    const {data} = await $host.get('api/recipe/getRecipes', {params: {page, limit}});
    return data;
}

export const getRecipeByTitle = async (title) => {
    const {data} = await $host.get('api/recipe/getOneRecipe/' + title);
    return data;
}
